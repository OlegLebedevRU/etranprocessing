...
int main(int argc, char* argv[]) {
    ...
    printf("===============================================================================\n");
    printf("  %s v%s - Diagnostic Console Agent (Role: %s)\n",
           L4CON_APP_NAME, L4CON_APP_VERSION, config.role);
    printf("  Target: %s:%d | Leo4Proxy Port: %d | Timeout: %ds\n",
           config.mqtt_host, config.mqtt_port, config.proxy_http_port, config.default_cmd_timeout);
    printf("  Blacklist: %s\n", config.enable_blacklist ? "ENABLED (dangerous cmds blocked)" : "DISABLED");
