...
struct l4c_color_converter {
    ...
    int32_t stride_y, stride_u, stride_v;
    uint8_t *plane_y;
    uint8_t *plane_u;
    uint8_t *plane_v;
    uint8_t *buffer; /* Single allocation for all 3 planes */
    size_t plane_y_size, plane_u_size, plane_v_size;
};
...
l4c_status_t l4c_color_converter_create(uint32_t width, uint32_t height, l4c_color_converter_t **out_converter) {
    ...
    total = cvt->plane_y_size + cvt->plane_u_size + cvt->plane_v_size;
