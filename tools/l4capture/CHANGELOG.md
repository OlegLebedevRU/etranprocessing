...
- Media Foundation encoder: ограничено ожидание событий, разведены input/output tokens и подтверждается фактический encoded output. Это устранило зависания и чёрный старт на терминале 773.
- DPI координаты и выбранный монитор согласованы с физическими пикселями; кадр 3000×2000 и второй экран передаются целиком.
- Краткие стартовые IDR ускоряют появление изображения. Ограничение QP аппаратного кодера и пик битрейта сохраняют читаемость текста при длительной трансляции без заметной задержки на 773.
- Уточнены RTCP timestamps, удержание DXGI frame при fallback и учёт RTP отправки.
