...
if errorlevel 1 (
    ...
    echo [OK] x86 build SUCCESS: bin\x86\l4sql.exe
    copy /y bin\x86\l4sql.exe bin\l4sql.exe >nul
    echo [OK] Copied universal x86 binary to: bin\l4sql.exe
)
exit /b 0
...
if errorlevel 1 (
    ...
    echo [OK] x64 build SUCCESS: bin\x64\l4sql.exe
)
exit /b 0
...
if %BUILD_FAILED% equ 0 (
    ...
    if exist bin\x86\l4sql.exe echo   - x86 [32-bit universal]: bin\x86\l4sql.exe
    if exist bin\x64\l4sql.exe echo   - x64 [64-bit]:           bin\x64\l4sql.exe
    if exist bin\l4sql.exe     echo   - Universal default:      bin\l4sql.exe
