...
- **Health-check и отказоустойчивость**:
  ...
  - **Инвариант одного процесса**: дескрипторы старого процесса и Job Object полностью освобождаются перед порождением нового процесса.
  - **Отмена при stream_stop**: при получении команды `stream_stop` любой запланированный restart немедленно отменяется (`next_restart_time = 0`), а процесс штатно останавливается.
  - **Локальный сторожевой таймер аренды (Fail-Closed Watchdog)**: отслеживание времени окончания аренды `expires_at_ms`. Если локальное время терминала превышает `expires_at_ms + 5000` (льготный период grace 5 секунд) и продления не поступило, супервизор выполняет принудительный fail-closed останов:
    1. Вызывает `input_release_all()` для сброса всех удерживаемых клавиш клавиатуры (`KEYEVENTF_KEYUP`) и кнопок мыши (`MOUSEEVENTF_LEFTUP`, `MOUSEEVENTF_RIGHTUP`, `MOUSEEVENTF_MIDDLEUP`).
