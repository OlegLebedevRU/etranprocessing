...
echo =======================================================
echo Building l4desk (C / MSVC /MT Static - Unified 32/64)
echo =======================================================
...
cmd /c ""%VS_DEV_CMD%" -arch=x86 -no_logo && rc.exe /nologo /fo obj\x86\l4desk.res res\l4desk.rc && cl.exe /nologo /O2 /MT /W4 /utf-8 /D_WIN32_WINNT=0x0601 /D_CRT_SECURE_NO_WARNINGS /D_WINSOCK_DEPRECATED_NO_WARNINGS /DWIN32_LEAN_AND_MEAN /I src /I res /Foobj\x86\ src\main.c src\config.c src\sn_discovery.c src\log.c src\json_min.c src\desktop_state.c src\display_inventory.c src\input_inject.c src\dedup_cache.c src\ctl_protocol.c src\mqtt_protocol.c src\mqtt_client.c src\ffmpeg_cmdline.c src\ffmpeg_supervisor.c obj\x86\l4desk.res /link /SUBSYSTEM:CONSOLE,6.01 /OUT:bin\x86\l4desk.exe ws2_32.lib winhttp.lib advapi32.lib user32.lib wtsapi32.lib ole32.lib oleaut32.lib gdi32.lib"
if errorlevel 1 (
    echo [ERROR] x86 build failed!
    set BUILD_FAILED=1
) else (
    echo [OK] x86 build SUCCESS: bin\x86\l4desk.exe
    copy /y bin\x86\l4desk.exe bin\l4desk.exe >nul
)
exit /b 0
...
cmd /c ""%VS_DEV_CMD%" -arch=x64 -no_logo && rc.exe /nologo /fo obj\x64\l4desk.res res\l4desk.rc && cl.exe /nologo /O2 /MT /W4 /utf-8 /D_CRT_SECURE_NO_WARNINGS /D_WINSOCK_DEPRECATED_NO_WARNINGS /DWIN32_LEAN_AND_MEAN /I src /I res /Foobj\x64\ src\main.c src\config.c src\sn_discovery.c src\log.c src\json_min.c src\desktop_state.c src\display_inventory.c src\input_inject.c src\dedup_cache.c src\ctl_protocol.c src\mqtt_protocol.c src\mqtt_client.c src\ffmpeg_cmdline.c src\ffmpeg_supervisor.c obj\x64\l4desk.res /link /OUT:bin\x64\l4desk.exe ws2_32.lib winhttp.lib advapi32.lib user32.lib wtsapi32.lib ole32.lib oleaut32.lib gdi32.lib"
if errorlevel 1 (
    echo [ERROR] x64 build failed!
    set BUILD_FAILED=1
) else (
    echo [OK] x64 build SUCCESS: bin\x64\l4desk.exe
)
exit /b 0
...
if %BUILD_FAILED% equ 0 (
    ...
    if exist bin\x86\l4desk.exe echo   - x86 [32-bit]: bin\x86\l4desk.exe
    if exist bin\x64\l4desk.exe echo   - x64 [64-bit]: bin\x64\l4desk.exe
    if exist bin\l4desk.exe     echo   - Default:      bin\l4desk.exe

    for %%f in (l4desk_*.cmd) do (
        copy /y "%%f" bin\"%%f" >nul
