...
void schannel_close(SChannelSession* session) {
    ...
    if (SecIsValidHandle(&session->hCtx)) {
        ...
        SecBuffer outBuffer;
        outBuffer.cbBuffer = sizeof(dwType);
        outBuffer.BufferType = SECBUFFER_TOKEN;
        ...
        ApplyControlToken(&session->hCtx, &outDesc);

        DWORD reqFlags = ISC_REQ_SEQUENCE_DETECT | ISC_REQ_REPLAY_DETECT | ISC_REQ_CONFIDENTIALITY | ISC_REQ_ALLOCATE_MEMORY | ISC_REQ_STREAM;
        DWORD ctxFlags = 0;
        TimeStamp tsExpiry;

        SecBuffer shutdownBuffer;
        shutdownBuffer.BufferType = SECBUFFER_TOKEN;
        shutdownBuffer.cbBuffer = 0;
        shutdownBuffer.pvBuffer = NULL;
        ...
        if (InitializeSecurityContextA(
            ...
            if (shutdownBuffer.cbBuffer > 0 && shutdownBuffer.pvBuffer) {
                send_all(session->sock, (const BYTE*)shutdownBuffer.pvBuffer, (int)shutdownBuffer.cbBuffer);
                FreeContextBuffer(shutdownBuffer.pvBuffer);
            }
        }
