...
## 7. Горячая ротация сертификатов в рантайме (Service Mode Hot-Reloading)

В режиме службы Windows (`--service` / системный сервис `Leo4Proxy`) реализован автономный механизм неагрессивного мониторинга хранилища сертификатов (`LocalMachine\MY`):

1. **Фоновый поллинг хранилища (по умолчанию каждые 30 секунд)**:
   - Служба периодически опрашивает хранилище (`cert_store_find_best_cert`), отслеживая появление новых или заменённых сертификатов терминала без необходимости перезагрузки службы. Интервал настраивается через `--cert-poll-interval <sec>`.
2. **Горячая замена дескрипторов безопасности SChannel (Hot-Swap)**:
   - При обнаружении нового сертификата (изменение SHA-1 thumbprint) `leo4proxy` запрашивает новые `CredHandle` (Client mTLS и Server SSL).
   - **HTTP Proxy**: дескрипторы обновляются атомарно на лету, новые запросы сразу уходят с новым сертификатом и новым заголовком `X-Leo4-Proxy-Sn`.
   - **MQTT Proxy**: активные клиентские соединения кратковременно сбрасываются, провоцируя немедленное переподключение локального `mosquitto bridge` / клиентов с установлением mTLS-сессии по новому сертификату.
   - **Reverse HTTPS Proxy**: листенер перезапускается с новым серверным сертификатом.
   - **Discovery (mDNS / LLMNR)**: если изменился серийный номер `SN`, анонсы `leo4-<new_sn>.local` обновляются автоматически.
