...
### Added & Changed
- **Windows Defender Firewall для FFmpeg и видеостриминга**:
  - Добавлено правило брандмауэра `L4Tools-FFmpeg` для исполняемого файла `%dest%\ffmpeg\ffmpeg.exe` (`dir=in action=allow profile=any enable=yes`).
  - Добавлены сетевые порты локального стриминга UDP: `L4Tools-RTP-Port` (`5004/UDP`) и `L4Tools-RTCP-Port` (`5005/UDP`).
  - Обеспечена полная идемпотентность через предварительное удаление правил с тем же именем перед регистрацией.
  - Устранено всплывающее системное окно «Оповещение системы безопасности Windows» при вызове `bind()` сокетов RTCP в FFmpeg.
- **Тихая установка доверенного корневого CA-сертификата (`iot_leo4_ca.crt`)**:
  - Реализована встроенная установка сертификата CA `iot.leo4.ru` (SHA-1 thumbprint: `B0A01EB219110CAC1077DD5171EF42A442AE5A87`) в системное хранилище `LocalMachine\ROOT` через Win32 CryptoAPI без внешних утилит и диалогов.
  - Идемпотентная проверка существующего сертификата по отпечатку и валидности срока действия.
  - Вызов в фазах Preflight (Фаза 1, 2) и Cert Phase (Фаза 3/5) до инициации TLS-соединений и запуска служб.
  - Устранена ошибка валидации Schannel `SEC_E_UNTRUSTED_ROOT` (`0x80090325`).
- **Отчетность и метаданные установки**:
  - В `install_summary.json` и `summary.json` добавлены флаги `ca_root_installed: true` и `firewall_configured: true`.
  - Синхронизирована версия пакета инструментов до `1.7.5`.
