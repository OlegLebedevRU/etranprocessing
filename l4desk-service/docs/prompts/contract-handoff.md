...
## 51. Принятие H-L4D-17C-VIDEO-WATCH-MB-v1 (MenuBuilder watch consumer)

Контроллер независимо сверил опубликованный `DETACHED_V1` candidate в commit
`51137b979b01455b4af453a282a17063fea53ebe` (Git/raw SHA-256 candidate
`f514b5426cc69858ddb32727a7f45de786d3bb5ffff4b0fe3ff4d762ead5717a`),
отчёт в commit `5020aa3c568b966db29073c8c4e00f4d26b59fca` и все шесть указанных
Git/raw artifact digest. Входной provider `H-L4D-17C-VIDEO-WATCH-IOT-01-v1`
принят в §43, corrective зарегистрирован в §48 и уточнён по подтверждению
пользователя в §50. Активный MenuBuilder backend image и frontend index повторно
сверены; route/config на сервере являются точными CRLF-копиями Git/raw LF-файлов.
Backend suite: 477 passed, Ruff/format/Pyright clean; frontend build и 55 tests
passed. Оператор подтвердил кадры, Stop/Start и работающий WS терминала 773;
по серверному журналу при живом WS выполняется одна пара status GET за ~62 с.
Browser E2E fallback при сетевом отказе WS не воспроизведён, поскольку DevTools
не заблокировал WS-handshake; локальная проверка эффекта с принудительным отказом
показала REST-пару каждые 5 с и прекращение polling после восстановления WS.
Этот остаточный риск передаётся 17E/17F. Принятие corrective закрывает только
этот gate и не означает приёмку 17E или 17F.
...
accepted_at_utc: '2026-09-26T10:51:00Z'
contract_version: 1.0.0
schema_revision: 2026-09-26-v1
artifact_version: 1.0.0
candidate_format: DETACHED_V1
