...
Задача: устранить унаследованный от `H-L4D-08B-FIX-01-MB-v1` и не закрытый в `H-L4D-08B-FIX-02-MB-v1` residual risk — параллельный stop-path в `video_control.py` вне `RemoteSessionUseCase`. Оба существующих HTTP-сценария остановки должны использовать одну идемпотентную backend-операцию для сохранённого `provider_session_id`.
