...
Для возобновления передать агенту точный файл FIX из **нового** commit B и сообщение контроллера с полным SHA B/подтверждением push в `origin/l4desk/l4d-06a-pb`. В сообщении указать: выполнить обновлённый FIX по стандарту 1.2.0, регистрации v2, четырём разрешённым PIN-документам и IOT byte binding; не читать семь .py старого 06A; вернуть отчёт и detached candidate для повторной приёмки. `ddf39184...` не заменяет публикацию B. SHA B передаётся отдельно после commit, чтобы не создавать самоссылку в самом задании.
