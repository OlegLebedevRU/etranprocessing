...
| `L4D-17C-IOT` (ещё не начат) | `L4D-17C-VIDEO-WATCH-IOT-01` / [L4D-17C-VIDEO-WATCH-IOT-01.md](L4D-17C-VIDEO-WATCH-IOT-01.md) | `iot-rpc-rest-app` | `R-L4D-17C-VIDEO-WATCH-IOT-01-v1` | Read-only video state observer для одного worker; после provider acceptance возобновить `L4D-17C-IOT` |
...
Для `H-L4D-07-IOT-STOP-v1` применяется узкая проверка §11 стандарта: исходный блок из `42d0876` остаётся неизменным, а `C-H-L4D-07-IOT-STOP-v1-FEATURE-FLAGS-v1` восстанавливает только пропущенные `feature_flags`. Их значения совпадают с ранее принятым `H-L4D-07-IOT-v1`; без этой сверки consumer возвращает `BLOCKED_CONTRACT`. Уточнение не принимает FIX-03 и не продвигает основной каскад.
