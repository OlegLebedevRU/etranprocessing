...
|---|---|---|---|---|
| `L4D-06B-IOT` | `L4D-06B-IOT-FIX-01` / [etran_dev-l4d-06b-iot-fix-01.md](etran_dev-l4d-06b-iot-fix-01.md) | `iot-rpc-rest-app` | `R-L4D-06B-IOT-FIX-01-v1` | Повторная приёмка `L4D-06B-IOT`; не запуск `L4D-06C-MB` |

Регистрация и адресный допуск определяются §8 `PROMPT-STANDARD.md` и отдельной append-only записью `CORRECTIVE_REGISTRATION` в журнале. Принятые provider-контракты и их ID не меняются. Для этого FIX согласован `DETACHED_V1`: окончательный candidate находится отдельно от отчёта и содержит SHA-256 зафиксированных байтов отчёта (§9 стандарта).
