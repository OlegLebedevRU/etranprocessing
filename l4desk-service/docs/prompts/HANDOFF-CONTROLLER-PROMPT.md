...
## 2.1. Сверка накопленных изменений перед 17E и закрытием каскада

Перед запуском `L4D-17E-MB`, приёмкой `L4D-17F-DOCS` и закрытием `L4D-18F-DOCS` прочитай [контрольную точку перед 17E](L4D-17E-PRELAUNCH-CHECKPOINT.md) и **сверь её заново** с принятыми блоками журнала, опубликованными отчётами и фактическими версиями. Контрольная точка — индекс известных изменений и пробелов, а не новый `HANDOFF` или разрешение читать соседний исходный код исполнителю. Для каждой строки укажи: принятый source commit, более поздний commit/деплой, evidence, затронутый consumer, итог `COVERED`/`PENDING`/`DRIFT` и ссылку на принятый handoff либо corrective. Не переносить обещания из отчёта в статус `ACCEPTED`.

Для `L4D-17C-VIDEO-WATCH-MB` регистрация `R-L4D-17C-VIDEO-WATCH-MB-v1` только открывает corrective. Перед 17E требуются опубликованный отчёт со статусом `ACCEPTED`, отдельный `DETACHED_V1` candidate, независимая проверка хешей и фактического deployed commit по §9 стандарта и запись `H-L4D-17C-VIDEO-WATCH-MB-v1` в журнале. Нынешний `READY_FOR_BROWSER_VALIDATION` и отсутствие candidate не проходят этот gate. Браузерные логи о снижении polling подтверждают только эту часть поведения; они не заменяют E2E медиатракта, auth/tenant negative cases или полного набора проверок corrective.

Если после принятого handoff появились новые runtime commits, не переписывай старый блок и не считай их частью прежней приёмки. Отдельно зафиксируй provenance и проверь совместимость, digest, deployment и потребителей через применимый corrective/acceptance. При расхождении останови переход к зависимому шагу. В итоговый `L4D-18F-DOCS` перенеси сверенную матрицу версий и открытых рисков из контрольной точки; `CLOSED_ACCEPTED` допустим только при отсутствии незакрытых обязательных gates и drift.
