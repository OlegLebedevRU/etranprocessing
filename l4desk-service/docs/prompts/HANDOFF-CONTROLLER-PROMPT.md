...
**Единственный разрешённый scope изменений:** `l4desk-service` (файл `l4desk-service/docs/prompts/contract-handoff.md`).
...
5. **Проверка артефактов и расчет SHA-256:**
   - Для каждого артефакта из списка `artifact_paths` (включая сам файл отчёта):
     ...
     - Сверь рассчитанный хеш с хешем, заявленным в блоке handoff отчёта.
