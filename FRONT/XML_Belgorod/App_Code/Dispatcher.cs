...
public class Dispatcher : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        try
        {
            ...
            string sp_name = "Belgorod_GetEndDay";
            //string pref = "MED_";
            ...
            if (col["from"] != null)
                DateTime.TryParse(col["from"], out dt_from);
            if (col["to"] != null)
                DateTime.TryParse(col["to"], out dt_to);


            //int org_id = int.Parse(col["org_id"] == null ? "0" : col["org_id"]);
            string fileName =  dt_from.ToString("yyyyMMdd") + "-" + dt_to.ToString("yyyyMMdd") + ".xml";
