...
namespace Xml
{
    ...
    class XmlClass
    {
        ...
        static public void Insert(XmlDocument xmldoc, XmlNode xmlnode, List<KeyValuePair<string, string>> htbl)
        {
            foreach (KeyValuePair<string, string> pair in htbl)
            {
                string name = pair.Key;
                string val = pair.Value;

                string pattern = @"\[(.*?)\]";
                string query = val;// "H1-receptor antagonist [tsp_descr=fsdfsdfsfs] [tsp_code=5555]";
                ...
                if (Regex.IsMatch(val, pattern))
                {
                    ...
                    foreach (Match m in matches)
                    {
                        ...
                        string attrName = find.Split('=')[0];
                        string attrValue = find.Split('=')[1];
                        attr.Add(attrName,attrValue);
                ...
                XmlElement el = CreateElement(xmldoc, name, attr, val);
                xmlnode.AppendChild(el);
            }
        }
    }
    ...
    namespace Data
    {
        ...
        public class DBManager
        {
            ...
            public DBManager(string _constr)
            {
                ...
                connection.Open();
                SqlCommand arithabortCommand = new SqlCommand("SET ARITHABORT ON", connection);
