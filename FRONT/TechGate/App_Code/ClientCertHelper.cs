...
public static class ClientCertHelper
{
    ...
    public static string GetDBSerialNumberOrThrow(HttpContext ctx)
    {
        string serial = GetDBSerialNumber(ctx);
        if (string.IsNullOrEmpty(serial))
            throw new Exception("Missing or invalid client certificate: cannot determine DB serial number.");
        return serial;
    }
