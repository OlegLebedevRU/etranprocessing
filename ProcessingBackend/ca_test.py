...
def generate_test_csr(
    cn: str = "A1B2C3D4E5F6", device_id: int = 773, org_id: int = 223
) -> str:
...
DEFAULT_CA_CERT = os.path.join(SCRIPT_DIR, "keys", "ca.crt")
DEFAULT_CA_KEY = os.path.join(SCRIPT_DIR, "keys", "ca.key")
DEFAULT_CERTS_DIR = os.path.join(SCRIPT_DIR, "keys", "certs")


def setup_env(ca_cert: str, ca_key: str):
    os.environ["CA_CERT_PATH"] = ca_cert
    os.environ["CA_KEY_PATH"] = ca_key
    os.environ["CA_CERTS_DIR"] = DEFAULT_CERTS_DIR + "/"
    os.makedirs(DEFAULT_CERTS_DIR, exist_ok=True)
...
def verify_cert(
    ...
    expected_cn: str,
    expected_e: str,
    expected_san_uris: list[str] | None = None,
    expected_san_dns: list[str] | None = None,
):
    """Verify certificate fields. Returns True if all checks pass."""
    if result["statusCode"] != 200:
        print(
            f"  FAIL: statusCode={result['statusCode']}, error={result.get('body', {})}"
        )
        return False
    ...
    if cn == expected_cn:
        ...
        print(f"  CN:      {cn}  [FAIL] (expected {expected_cn})")
        ok = False
    ...
    try:
        ...
        if email == expected_e:
            ...
            print(f"  E:       {email}  [FAIL] (expected {expected_e})")
            ok = False
    except Exception:
        print("  E:       NOT FOUND  [FAIL]")
        ok = False
    ...
    try:
        ...
        san_dns = [str(d) for d in san.value.get_values_for_type(x509.DNSName)]
    except x509.ExtensionNotFound:
        pass

    if expected_san_uris:
        for exp_uri in expected_san_uris:
            if exp_uri in san_uris:
                print(f"  SAN URI: {exp_uri}  [OK]")
            else:
                print(f"  SAN URI: {san_uris}  [FAIL] (missing {exp_uri})")
                ok = False

    if expected_san_dns:
        for exp_d in expected_san_dns:
            if exp_d in san_dns:
                print(f"  SAN DNS: {exp_d}  [OK]")
            else:
                print(f"  SAN DNS: {san_dns}  [FAIL] (missing {exp_d})")
                ok = False
...
def test_new_flow(ca_cert: str, ca_key: str):
    ...
    print("=" * 60)
    print("NEW FLOW (X-Cn present -> CN override + SAN URI & DNS)")
    print("=" * 60)

    csr_pem = generate_test_csr(
        cn="A1B2C3D4E5F6", device_id=4625
    )  # terminal puts sign as CN
    ...
    ok = verify_cert(
        result,
        expected_cn="a2b0004625c18468d030726",
        expected_e="1.terminal@forpay.ru",
        expected_san_uris=["a2b0004625c18468d030726", "urn:sign:A1B2C3D4E5F67890"],
        expected_san_dns=["leo4-0004625.local"],
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ca-key", default=DEFAULT_CA_KEY)
    ...
    print("=" * 60)
    print(
        f"SUMMARY: new_flow={'PASS' if r1 else 'FAIL'}, old_flow={'PASS' if r2 else 'FAIL'}"
    )
    print("=" * 60)
