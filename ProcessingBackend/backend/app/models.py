...
class TspParameterCode(Base):
    ...
    __table_args__ = (
        UniqueConstraint("prototypenumber", "parameter_code", name="uq_prototype_param_code"),
...
class Payment(Base):
    ...
    paym_datetime: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    ...
    org_id: Mapped[int] = mapped_column(Integer, ForeignKey("orgs.org_id"), nullable=False)
    ...
    params: Mapped[list["PaymentParam"]] = relationship(back_populates="payment", cascade="all, delete-orphan")
...
class PaymentParam(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    paym_id: Mapped[int] = mapped_column(Integer, ForeignKey("payments.paym_id", ondelete="CASCADE"), nullable=False)
    param_id: Mapped[int] = mapped_column(Integer, ForeignKey("tsp_parameter_codes.param_id"), nullable=False)
...
class BalanceTerminalTsp(Base):
    ...
    org_id: Mapped[int] = mapped_column(Integer, ForeignKey("orgs.org_id"), nullable=False)
    terminal_id: Mapped[int] = mapped_column(Integer, ForeignKey("terminals.id"), nullable=False)
    tsp_id: Mapped[int] = mapped_column(Integer, ForeignKey("tsp.tsp_id"), nullable=False)
    amount: Mapped[int] = mapped_column(BigInteger, default=0)
    count: Mapped[int] = mapped_column(Integer, default=0)
    update_datetime: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())

    __table_args__ = (
        UniqueConstraint("int_day", "terminal_id", "tsp_id", name="uq_balance_day_terminal_tsp"),
