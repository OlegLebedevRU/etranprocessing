...
from app.models import Group, MenuVariant, ServiceMenu, TerminalMenuBinding
...
@pytest.mark.anyio
async def test_list_menu_file_by_device_id_query_param():
...
@pytest.mark.anyio
async def test_list_menu_file_by_mtls_cert_dn_header():
    variant, binding, groups, services = _setup_menu_data()
    ...
    headers = {"X-Client-Cert-DN": "CN=term-001,OU=773,O=1,C=RU"}

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.get("/api/ListMenuFile", headers=headers)
