...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint('monthly_price_minor >= 0', name='ck_org_billing_price_non_negative'),
    )
    ...
    op.create_index('idx_billing_order_items_terminal_id', 'billing_order_items', ['terminal_id'])

    # --- extend licenses table ---
    # Use server_default for existing rows, then remove default
    op.add_column('licenses', sa.Column(
        'billing_period_months', sa.SmallInteger(), server_default='1', nullable=False
    ))
    op.add_column('licenses', sa.Column(
        'monthly_price_override_minor', sa.BigInteger(), nullable=True
    ))
    op.add_column('licenses', sa.Column(
