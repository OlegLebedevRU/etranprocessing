...
def upgrade() -> None:
    ...
    op.add_column(
        "org_billing_settings",
        sa.Column("cert_price_minor", sa.BigInteger(), nullable=True),
    )
    op.add_column(
        "org_billing_settings",
        sa.Column(
    ...
    op.create_index("idx_cert_pins_order_item", "certificate_pins", ["order_item_id"])
    op.execute(
...
def downgrade() -> None:
    op.drop_index(
        "idx_terminal_cert_history_terminal", table_name="terminal_cert_history"
    )
    ...
    op.drop_constraint(
        "fk_cert_pins_order_item", "certificate_pins", type_="foreignkey"
    )
