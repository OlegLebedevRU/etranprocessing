...
typedef enum {
    CONN_TYPE_INGRESS_LISTEN,
    CONN_TYPE_INGRESS_CLIENT,
    CONN_TYPE_CONTROL_LISTEN,
    CONN_TYPE_CONTROL_CLIENT
} ConnType;
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(p, "%127s %d %d", sn, &rtp, &rtcp) == 3) {
            if (g_routes.count < MAX_ROUTES) {
                strncpy(g_routes.entries[g_routes.count].sn, sn, sizeof(g_routes.entries[0].sn) - 1);
...
static bool upsert_route(const char* sn, int rtp, int rtcp) {
    for (int i = 0; i < g_routes.count; i++) {
        if (strcmp(g_routes.entries[i].sn, sn) == 0) {
...
static bool delete_route(const char* sn) {
    for (int i = 0; i < g_routes.count; i++) {
        if (strcmp(g_routes.entries[i].sn, sn) == 0) {
...
static bool resolve_janus_host(void) {
    struct hostent* he = gethostbyname(g_janus_host);
    if (!he || !he->h_addr_list || !he->h_addr_list[0]) {
        fprintf(stderr, "[INGRESS] Warning: Could not resolve '%s' via DNS, trying fallback 127.0.0.1\n", g_janus_host);
        inet_pton(AF_INET, "127.0.0.1", &g_janus_ip);
        return false;
    }
    memcpy(&g_janus_ip, he->h_addr_list[0], sizeof(struct in_addr));
    char ip_str[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &g_janus_ip, ip_str, sizeof(ip_str));
    printf("[INGRESS] Resolved target host '%s' -> %s\n", g_janus_host, ip_str);
    return true;
}
...
static IngressClient* add_ingress_client(int epoll_fd, int client_fd, const char* peer_ip, int peer_port) {
    ...
    c->fd = client_fd;
    c->type = CONN_TYPE_INGRESS_CLIENT;
...
static void remove_ingress_client(int epoll_fd, IngressClient* c) {
    ...
    printf("[INGRESS] Disconnected %s (SN: '%s', duration: %lds, RTP: %lu pkts, RTCP: %lu pkts, Total: %lu bytes)\n",
           c->peer_addr, c->sn[0] ? c->sn : "unknown", (long)duration,
           (unsigned long)c->rtp_packets, (unsigned long)c->rtcp_packets,
...
static void process_ingress_data(int epoll_fd, IngressClient* c) {
    while (true) {
        ...
        if (c->state == STATE_PREAMBLE) {
            ...
            if (!route) {
                fprintf(stderr, "[INGRESS] Unknown SN '%s' from %s — rejecting connection\n", c->sn, c->peer_addr);
                remove_ingress_client(epoll_fd, c);
                return;
            }
        ...
        if (c->state == STATE_FRAMING) {
            while (c->rx_len >= 4) {
                uint8_t type = c->rx_buf[0];
                uint16_t payload_len = ((uint16_t)c->rx_buf[2] << 8) | (uint16_t)c->rx_buf[3];
                size_t frame_total = 4 + payload_len;

                if (c->rx_len < frame_total) {
                    /* Incomplete frame, wait for more data */
                    break;
                }

                const uint8_t* payload = c->rx_buf + 4;
                ...
                memmove(c->rx_buf, c->rx_buf + frame_total, c->rx_len - frame_total);
                c->rx_len -= frame_total;
            }
        }
    }
}
...
static void handle_control_request(int client_fd) {
    ...
    char resp_body[HTTP_BUFFER_SIZE];
    resp_body[0] = '\0';
    int status_code = 200;
    ...
    if (strcmp(method, "GET") == 0 && strcmp(path, "/routes") == 0) {
        /* GET /routes -> JSON */
        size_t off = 0;
        off += snprintf(resp_body + off, sizeof(resp_body) - off, "[");
        for (int i = 0; i < g_routes.count; i++) {
            off += snprintf(resp_body + off, sizeof(resp_body) - off,
        ...
        snprintf(resp_body + off, sizeof(resp_body) - off, "]\n");
    }
    else if ((strcmp(method, "PUT") == 0 || strcmp(method, "POST") == 0) &&
             strncmp(path, "/routes/", 8) == 0) {
        /* PUT /routes/<sn>?rtp=6002&rtcp=6003 */
        char sn[128] = {0};
        char* qmark = strchr(path + 8, '?');
        int rtp = 0, rtcp = 0;

        if (qmark) {
            size_t sn_len = qmark - (path + 8);
            if (sn_len < sizeof(sn)) {
                strncpy(sn, path + 8, sn_len);
                sn[sn_len] = '\0';
            }
            char* p_rtp = strstr(qmark, "rtp=");
            char* p_rtcp = strstr(qmark, "rtcp=");
            if (p_rtp) rtp = atoi(p_rtp + 4);
            if (p_rtcp) rtcp = atoi(p_rtcp + 5);
        } else {
            strncpy(sn, path + 8, sizeof(sn) - 1);
        }

        if (sn[0] && rtp > 0 && rtcp > 0) {
            upsert_route(sn, rtp, rtcp);
            snprintf(resp_body, sizeof(resp_body),
                     "{\"status\":\"ok\",\"action\":\"upsert\",\"sn\":\"%s\",\"rtp_port\":%d,\"rtcp_port\":%d}\n",
                     sn, rtp, rtcp);
        } else {
            status_code = 400;
        ...
        for (int i = 0; i < g_client_count; i++) {
            ...
            off += snprintf(resp_body + off, sizeof(resp_body) - off,
                            ...
                            "\"rtp_packets\":%lu,\"rtcp_packets\":%lu,\"bytes\":%lu,"
                            "\"duration_sec\":%ld,\"idle_sec\":%ld}",
                            (i > 0 ? "," : ""),
                            c->sn[0] ? c->sn : "pending",
                            c->peer_addr,
                            c->state == STATE_FRAMING ? "streaming" : "preamble",
                            c->rtp_port, c->rtcp_port,
                            (unsigned long)c->rtp_packets,
                            (unsigned long)c->rtcp_packets,
                            (unsigned long)c->total_bytes,
        ...
        status_code = 404;
        status_text = "Not Found";
        snprintf(resp_body, sizeof(resp_body), "{\"error\":\"endpoint not found\"}\n");
    }
...
int main(int argc, char* argv[]) {
    ...
    if (env_janus && strlen(env_janus) > 0) {
        strncpy(g_janus_host, env_janus, sizeof(g_janus_host) - 1);
    }
    const char* env_routes = getenv("ROUTES_FILE");
    if (env_routes && strlen(env_routes) > 0) {
        strncpy(g_routes_file, env_routes, sizeof(g_routes_file) - 1);
    }

    if (argc > 1) strncpy(g_routes_file, argv[1], sizeof(g_routes_file) - 1);

    printf("[INGRESS] Starting l4media-ingress server (alpha MVP)...\n");
    ...
    while (g_running) {
        ...
        if (now - last_stats_log >= 10) {
            ...
            if (g_client_count > 0) {
                ...
                for (int i = 0; i < g_client_count; i++) {
                    IngressClient* c = g_clients[i];
                    printf("  - SN: %-24s | %s | RTP: %lu pkts | RTCP: %lu pkts | %lu KB\n",
                           c->sn[0] ? c->sn : "(preamble)", c->peer_addr,
                           (unsigned long)c->rtp_packets, (unsigned long)c->rtcp_packets,
                           (unsigned long)(c->total_bytes / 1024));
                }
                fflush(stdout);
            }
        }
