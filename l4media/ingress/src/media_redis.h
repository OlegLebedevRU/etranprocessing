...
/* session_id|operation_id|sn|device_id|mountpoint|rtp|rtcp|pin|state|ttl|created|started|renewed */
...
static inline void media_redis_save_session(const char* session_id, const char* operation_id,
    ...
    char val[512];
    snprintf(val, sizeof(val), "%s|%s|%s|%u|%u|%d|%d|%s|%d|%d|%ld|%ld|%ld",
             session_id ? session_id : "",
             operation_id ? operation_id : "",
             sn ? sn : "",
             device_id, mountpoint_id, rtp_port, rtcp_port,
             pin_out,
             state, ttl_sec, (long)created_at, (long)started_at, (long)renewed_at);
...
static inline int media_redis_restore_sessions(media_redis_restore_fn fn, void* user) {
    ...
    for (size_t i = 0; i < r->elements; i++) {
        ...
        fn(fields[0], fields[1], fields[2],
           ...
           nf >= 13 ? (time_t)atol(fields[12]) : (time_t)atol(fields[11]),
           user);
        n++;
    }
    freeReplyObject(r);
