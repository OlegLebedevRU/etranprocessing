...
### 7.1. Концепция и соответствие общему Archive Manifest Contract v1
Подсистема архивации `l4media` реализует регламент ежемесячной архивации высокообъёмных технических метрик и телеметрии согласно общему контракту платформы L4Desk (`H-L4D-15A-DOCS-v1`).

1. **Типы архивируемых данных (`owner_project: l4media`):**
   * `media_stream_samples`: Периодические технические семплы медиапотока (битрейт, fps, rtp/rtcp-пакеты, переданные байты, джиттер, потери пакетов, rtt, разрешение кадра).
   * `media_quality_events`: Технические события качества сессии (деградация потока, всплески потерь, заморозки кадров, запросы ключевых кадров, адаптация битрейта).
2. **Горячие данные (STRICT HOT INVARIANT — никогда не удаляются архиватором):**
   * Таблицы маршрутизации Ingress (`routes.conf`, dynamic routing table).
   * Сводные строки и метаданные завершённых сессий (`tb_media_session_summaries`: `session_id`, `sn`, `duration`, `final_rtp_packets`, `final_bytes`, `stop_reason`).
   * Активные медиапотоки и данные, необходимые для активного обслуживания и реконсиляции (`POST /api/v1/media/reconcile`).
3. **Граница трёх полных месяцев (Retention Boundary):**
   Архивации и очистке подлежат только закрытые календарные месяцы старше 3 полных месяцев относительно текущей даты (например, при текущем месяце `2026-09` архивации подлежат месяцы `<= 2026-05`). Попытка архивации более свежего месяца блокируется защитным барьером `RetentionGuard` с ошибкой `HOT_RETENTION_VIOLATION`.

### 7.2. Жизненный цикл архивного пакета (Deterministic Lifecycle)
Каждый архивный пакет формируется по строгому детерминированному пайплайну:
```text
1. Retention Guard (> 3 полных месяцев) & Path Security Check
   │
2. Active Streams Guard (запрет при активных сессиях за архивируемый период)
   │
3. Экспорт в детерминированные gzip JSONL (mtime=0.0):
   • samples.jsonl.gz (media_stream_samples в обёртке RecordEnvelope)
   • quality.jsonl.gz (media_quality_events в обёртке RecordEnvelope)
   │
4. Расчёт SHA-256, создание checksum.sha256 и manifest.json (state="prepared")
   │
5. Криптографическая верификация (Verification Guard):
   • Полное перечитывание файлов и сверка SHA-256
   • Сверка количества строк и типов записей
   • Выборочная декомпрессия и валидация RecordEnvelope (restore_sample_status="passed")
   • Проверка курсоров (cursor_guard_passed=true)
   │
6. Атомарное перемещение (os.replace) из промежуточного .tmp_<batch_id>
   в канонический путь: <volume_root>/<year>/<month>/l4media/<batch_id>/
   (state="verified")
   │
7. Удаление (Purge Phase, только если dry_run=false и purge=true):
   • Повторная проверка отсутствия активных потоков
   • Порционное удаление проверенных записей из горячего хранилища (bounded chunks)
   • Фиксация состояния в манифесте: state="purged", purge_status="completed"
```

### 7.3. Инвариант No-Purge-On-Mismatch
При обнаружении любого несоответствия (несовпадение контрольной суммы SHA-256, расхождение счётчиков записей, ошибка декомпрессии/валидации RecordEnvelope, активная сессия в диапазоне дат или сбой дискового тома) процедура очистки категорически запрещена. Манифест помечается состоянием `state: "failed"` с деталями ошибки, а данные горячего хранилища остаются нетронутыми.

### 7.4. Требования к хранению и резервному копированию (Retention Policy)
* Срок хранения архивов: минимум 3 года (`retention_years: 3`).
* Обязательность резервного копирования: `backup_required: true`.
* Доступен инструмент аудита и верификации резервных копий без модификации данных: `python -m archive.cli check-retention`.
