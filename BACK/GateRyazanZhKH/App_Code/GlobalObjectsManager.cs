...
public sealed class GlobalObjectsManager
{
    ...
    [DllImport(@"C:\PROCESSING\GateRyazanZhKH\DLL.dll")]
    ...
    [DllImport(@"C:\PROCESSING\GateRyazanZhKH\DLL.dll", CallingConvention = CallingConvention.StdCall,CharSet = CharSet.Ansi)]
        ...
        try
        {
            switch (method)
            {
                ...
                case enKVCMethod.Distribute:
                    _KvcLastError = 0;
                    ret = Distribute((int)list[0], out _KvcLastError);
                    break;
