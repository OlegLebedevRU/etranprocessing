...
## 1. Статус и итоги выполнения Шага 4

Все задачи и требования ревизии контрактов Шага 4 (F1–F9) полностью **ВЫПОЛНЕНЫ**, протестированы локальными/регрессионными наборами тестов и задеплоены на прод-сервер `87.242.100.34`:
- **tools/l4desk (v1.5.0):** Устранена первопричина отбрасывания `lease_renew` (F1/F2), добавлена строгая проверка эпохи `stream_instance_id` (NACK `stream_mismatch`) и валидация дедлайна `expires_at_ms` (NACK `invalid_payload`), сессионный мьютекс переведен на формат с явным серийным номером (`Local\L4Desk_SingleInstance_<SN>`), собраны статические артефакты `/MT` x86 и x64.
- **l4media:** Устранена проблема ложного статуса STREAMING (F5) — ingress раздельно отслеживает `last_rtp_time` и `last_activity` с порогом `RTP_STALE_DEADLINE_SEC = 10`, введены монотонная эпоха соединения `connection_epoch` и enum `media_state`, добавлены эндпоинты `/stats` и `/stats/<sn>`, автоматические C unit-тесты (`make test`) и Python regression suite (6 сценариев). Уровень отладки Janus понижен до `debug_level = 3` (F9, исключена утечка PIN в stdout), логи Nginx направлены в `/dev/stdout` и `/dev/stderr`.
- **MenuBuilder frontend:** Реализована архитектура `SessionLifecycleCoordinator` (F3/F4) с трекингом поколений (`generation`), неперекрывающимся расписанием keepalive, фильтрацией событий по `stream_instance_id`, безопасным detach input без освобождения общей аренды (`sharedLease`) и сбросом зажатых клавиш (`key_event` kind="up") при teardown; тесты на Vitest с fake timers пройдены, production-бандл собран и задеплоен.
- **MenuBuilder backend:** Нормализован контракт ошибок 404 (`{"code": "lease_not_found", ...}`) с полями `lease_id` и `generation`, поддержана передача `generation` в `remote_input_keepalive`. Переработана идемпотентность `stop_device_stream` с сохранением 409 для реальных конфликтов (owner, tenant, epoch). Статус видеопотока синхронизирован со свежими RTP и серверной арендой; сервис протестирован и задеплоен.
- **iot-rpc-rest-app (app1):** Поле `CtlLeaseRenew` приведено к каноническому wire-полю `command_id` (UUID) (F1/F2). Поддержана семантика состояний keepalive (`renew_status`: `server_accepted`, `terminal_applied`, etc.) и `applied_deadline_ms`. Идемпотентный release для владельца, защита режима камеры от сброса в десктоп при re-acquire/upgrade, изоляция эпох стримов. Все 101 тест пройдены, контейнер задеплоен на `87.242.100.34`.

Временные файлы на сервере в каталоге `/home/user1/contract-audit-20260911-1129` удалены. Следующий этап — сквозная стендовая верификация (E2E) с физическим/виртуальным терминалом.

### Task intake

- Тип задачи: ревизия и синхронизация контрактов, устранение инцидента 2026-09-11 (F1–F9).
...
- Поток: UI → BFF → app1 → RabbitMQ / терминальный Mosquitto → l4desk → ACK/event → app1 → BFF → UI.
...
- Статус: **ВЫПОЛНЕНО В КОДЕ И ЗАДЕПЛОЕНО**. Все целевые компоненты задеплоены на сервере 87.242.100.34.
...
## 3. Реестр проблем, владельцев и статус устранения

| ID / приоритет | Проблема | Статус устранения в Шаге 4 | Владелец |
|---|---|---|---|
| F1 / P0 | CtlLeaseRenew хранил `cmd_id`, а агент ожидал `command_id` UUID | **Устранено.** `command_id` UUID стал каноническим wire-полем в `app1`, поддержан в `l4desk` | app1 + l4desk |
| F2 / P0 | Отсутствие ожидания и корреляции ACK продления аренды; сброс пустых UUID | **Устранено.** Поддержаны `renew_status` (`server_accepted`, `terminal_applied`, etc.) и дедлайн `applied_deadline_ms` | app1 + l4desk |
| F3 / P1 | Преждевременный DELETE lease при отключении ввода и последующий 404 на keepalive | **Устранено.** В UI внедрен `SessionLifecycleCoordinator` с трекингом `generation` и безопасным `detachInput()` (`sharedLease`) | UI + BFF + app1 |
| F4 / P1 | Рассинхрон состояния стрима при остановке и устаревшие события прошлых эпох | **Устранено.** Фильтрация событий по `stream_instance_id` в UI/BFF и строгая проверка эпохи в `l4desk` | UI + BFF + l4desk |
| F5 / P1 | Ложный статус STREAMING без свежих RTP пакетов | **Устранено.** В `l4media-ingress` разделены `last_rtp_time` и `last_activity` (порог 10 с, enum `media_state`, `/stats/<sn>`) | l4media + BFF + UI |
| F6 / P1 | Несогласованность и отсутствие строгой проверки срока действия аренды | **Устранено.** Строгая валидация `expires_at_ms > now_ms` в `l4desk` (NACK `invalid_payload`) | app1 + l4desk |
| F7 / P2 | Конфликты сессионного мьютекса при запуске тестов агента на рабочем хосте | **Устранено.** Сессионный мьютекс `l4desk` изолирован по серийному номеру: `Local\L4Desk_SingleInstance_<SN>` | tools/l4desk |
| F8 / P2 | Ошибки MQTT provisioning и ACL у других терминалов | Проверено: изолировано от SN 773, triage не затрагивает ctl топики активного стрима | Ops + app1 |
| F9 / P1 | Логирование PIN и тел запросов в stdout контейнера Janus | **Устранено.** Уровень отладки Janus понижен до `debug_level = 3`, логи Nginx перенаправлены в stdout/stderr | l4media + Ops |
