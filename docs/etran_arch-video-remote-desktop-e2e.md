...
8. При штатном завершении BFF подтверждает остановку потока в app1, затем вызывает сервисный `POST /api/v1/media/sessions/stop` с SN, ID аренды и ID потока. Ingress удаляет маршрут и mountpoint только при совпадении ID своей активной медиасессии с одним из этих ID; старый stop не затрагивает новую сессию. Повторный вызов идемпотентен; после этого освобождается аренда и закрываются control WS и Janus-сессия. При разрыве связи на терминале срабатывает локальный fail-closed watchdog (5 с grace), принудительно останавливающий FFmpeg и сбрасывающий удерживаемый ввод через `input_release_all()`. При аварийном перезапуске агента супервизор выполняет `reconcile` (убийство осиротевшего процесса FFmpeg) и гарантированно отправляет на сервер событие `stream_event(stopped, agent_restart_reconcile)`.
