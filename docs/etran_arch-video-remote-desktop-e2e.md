...
**Срез исходников:** 11.09.2026. Актуализировано по результатам синхронизации контрактов `ctl v1` (R1 / P0), реализации Route-before-Start & Autonomous Recovery (Шаг 2), а также стабилизации цикла продления аренды (`lease_renew`), идемпотентности останова, исправления UX ввода и Reconcile-оповещений (Шаг 3) между `iot-rpc-rest-app` (`app1`), `MenuBuilder` (BFF/UI) и `tools/l4desk`.
...
7. **Сквозное удержание аренды (Keepalive Chain):** UI каждые 5 секунд вызывает `POST /api/v1/video/devices/{device_id}/control/keepalive`. BFF транслирует запрос в `app1`, который продлевает срок аренды и публикует команду `lease_renew` в топик `srv/{SN}/ctl`. Агент `l4desk` обновляет локальный таймер `g_sup.lease_expires_at_ms`. Это предотвращает ложную остановку энкодера локальным fail-closed watchdog'ом (5 с grace) при нормальной сессии.
...
- **Текущее состояние системы:** код всех звеньев (Шаги 1, 2 и 3) реализован и задеплоен; **ожидает выполнения сквозная стендовая верификация (Этап 4)**: запуск без unrouted пакетов, kill-restart, timeout-stop.
- **Следующие шаги:**
...
> ⏳ **Текущее состояние реализации (Шаги 1, 2 и 3):** Задачи R1–R6, а также стабилизация удержания аренды (`lease_renew`), идемпотентность останова, Reconcile-оповещения и исправления UX ввода полностью реализованы на уровне исходного кода, сервисы `app1` и `menubuilder-backend` с фронтендом задеплоены на сервере `87.242.100.34`, бинарники `l4desk` собраны. **Этап 4 (Сквозная E2E-верификация изменений на стенде/сервере) пока не выполнен** и является ближайшим следующим шагом.
