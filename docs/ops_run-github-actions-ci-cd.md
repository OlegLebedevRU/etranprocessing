...
> Для бета-варианта выбран [автономный builder](ops_run-beta-ci-cd.md).
> Workflow отключён в коде (`changes.if: false`), чтобы не получить второй
> источник production-деплоя после восстановления billing. Команды запуска
> GitHub ниже относятся к предыдущему варианту, не к активируемому бета-flow.
...
Причина подтверждена владельцем аккаунта через интерфейс GitHub:

> The job was not started because recent account payments have failed or your
> spending limit needs to be increased. Please check the 'Billing & plans'
> section in your settings.

Это billing-блокировка аккаунта; REST API не предоставил job/check-run/log с
причиной. Владелец должен проверить failed payments и Actions budgets/spending
limits в платёжных настройках GitHub. Сообщение не уточняет, какое из этих двух
условий сработало. Платёжные данные и лимиты агент самостоятельно не меняет.
Изменение YAML или переустановка runner не устраняют эту блокировку. После её
снятия повторить ручной выпуск `menubuilder-backend` с `deploy=true`; новый push
для повторного запуска не требуется. До этого прямым деплоем не подменять
неподтверждённый end-to-end flow через Actions.
...
В бета-реализации backend Dockerfile переведены на `uv.lock`, а базовые образы
зафиксированы digest. Политика уникальных тегов и повторного использования
опубликованного артефакта описана в новом бета-регламенте.
