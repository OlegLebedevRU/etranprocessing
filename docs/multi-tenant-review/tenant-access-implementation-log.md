...
### Что осталось:
- Все шаги программы успешно завершены.

---

## Шаг 14. Финальный отчёт и статус замечаний архитектурного ревью

**Дата:** 2026-09-03  
**Статус:** Выполнен  

### 1. Таблица статусов архитектурных дефектов (D1–D12)

| # | Где | Что было не так | Статус | Как решено |
|---|---|---|---|---|
| **D1** | `app/auth.py::get_current_user` | Доверие `X-User-*` заголовкам без токена (обход auth) | **РЕШЕНО** | Добавлен флаг `trust_proxy_identity_headers=False`. Заголовки игнорируются, без токена возвращается `401 Unauthorized`. |
| **D2** | `app/auth.py`, `app/user_store.py` | Хардкод `o.lebedev` как суперюзера в коде | **РЕШЕНО** | Удален весь хардкод имени пользователя. Привилегии определяются строго из БД / `auth_users` и claims токена. |
| **D3** | `app/services/jwt_issuer.py` | Нет retry и дедупликации параллельных вызовов к issuer'у | **РЕШЕНО** | Внедрен single-flight механизм на `asyncio.Future` по `(user_id, org_id)` и retry при 5xx / сетевых ошибках. |
| **D4** | `app/routers/auth.py::refresh_token` | `org` брался из истёкшего токена → суперюзер терял тенант | **РЕШЕНО** | Сессия стала источником истины: активный тенант извлекается из `user_sessions.active_org_id`. |
| **D5** | `app/routers/admin_tenants.py::switch` | Каждый switch создавал новую сессию и переписывал куку | **РЕШЕНО** | Сессия сохраняется при switch, в ней обновляется `active_org_id`; при refresh сессия ротируется на месте. |
| **D6** | `app/auth.py::create_master_token` | Локальная подпись master token (второй контур доверия) | **РЕШЕНО** | Удалена концепция master-токена. Все токены (включая платформенные с `orgId=0`) выпускаются внешним RS256 issuer'ом. |
| **D7** | `frontend/src/routes/login.tsx` | Хранение `mb_master_token`, `mb_is_superuser` в localStorage | **РЕШЕНО** | Удалено хранение токенов в localStorage. Фронтенд опирается на `SessionContext` и `/api/auth/me`. |
| **D8** | `app/config.py` | `jwt_expire_minutes=15` вызывал частую ротацию | **РЕШЕНО** | Установлены согласованные с issuer v2 значения TTL: 60 минут access, 30 дней refresh. |
| **D9** | `frontend/src/api/client.ts` | Только реактивный refresh по 401; пользователь ждал issuer | **РЕШЕНО** | Внедрен упреждающий silent refresh за 300 секунд до истечения токена и по событию `visibilitychange`. |
| **D10** | `client.ts` + `_set_auth_cookies` | Два параллельных транспорта токена (localStorage + cookie) | **РЕШЕНО** | Браузер переведен исключительно на HttpOnly cookies (`accessToken` path `/`, `refreshToken` path `/api/auth`). CSRF через `X-Requested-With`. |
| **D11** | Тенантные роутеры backend | Не везде `require_tenant_context`; `org_id` из клиента | **РЕШЕНО** | Роутеры защищены `require_tenant_context`, внедрен `resolve_org_id`, логируется аудит имперсонации. |
| **D12** | Документация | Устарела (HS256, master-токен, старый транспорт) | **РЕШЕНО** | Полностью переписаны `docs/multi-tenant-auth-architecture.md`, `docs/menubuilder-frontend-architecture.md`, `docs/database-ownership.md`. |

---

### 2. Таблица статусов замечаний ревью (B1/B2, H1–H3, M1–M4)

| Код | Уровень | Краткое описание замечания | Статус | Подтверждение решения |
|---|---|---|---|---|
| **B1** | BLOCKER | Отсутствовали миграции биллинга; alembic не в зависимостях | **РЕШЕНО** | Миграции централизованы в `ProcessingBackend`, на проде применен `alembic upgrade head` (актуальная ревизия `021`). |
| **B2** | BLOCKER | Платёжный тракт нефункционален end-to-end | **РЕШЕНО** | Интегрирован платёжный процессинг, транзакционное продление лицензий и снятие блокировок. |
| **H1** | HIGH | Разрыв XML-контракта терминального API при админ-блокировке | **РЕШЕНО** | Проверка `is_active` вынесена из слоя auth, терминал получает корректный XML `<Result>OK</Result><state>error</state>`. |
| **H2** | HIGH | Календарный дрейф при итеративном сложении месяцев | **РЕШЕНО** | Реализована фиксация `anchor_day` от исходной даты; тесты `test_h2_*` полностью подтверждают отсутствие дрейфа. |
| **H3** | HIGH | Рассогласование «оплачено N периодов ⇄ выдано N периодов» | **РЕШЕНО** | Формулы продления согласованы между расчетным сервисом и списаниями. |
| **M1** | MEDIUM | Просроченные терминалы исключались из месяцев прогноза | **РЕШЕНО** | Исправлен алгоритм прогнозирования в `app/services/billing.py`. |
| **M2** | MEDIUM | Терминал без лицензии переходил в статус active | **РЕШЕНО** | Синхронизирована машина состояний терминальных лицензий. |
| **M3** | MEDIUM | Checkout без блокировок, идемпотентности и транзакций | **РЕШЕНО** | Добавлены блокировки и транзакционное выполнение операций checkout. |
| **M4** | MEDIUM | Границы прогнозных месяцев рассчитывались по UTC | **РЕШЕНО** | Расчёты выверены в соответствии с часовым поясом организации и бизнес-календарем. |

---

### 3. Требования к развёртыванию (Deployment Requirements)

1. **Конфигурация окружения (.env)**:
   - `JWT_ISSUER_MOCK_ENABLED=False` (или не задан) — критично для предотвращения атак обхода аутентификации через HS256 mock_secret.
   - `JWT_VERIFY_AUDIENCE=True` — обязательная валидация claims `aud="menubuilder"` и `iss="external-jwt-issuer"`.
   - `SERVICE_TO_YC_SERVICE_SECRET` — обязан совпадать с `SERVICE_HMAC_SECRET` внешней Yandex Cloud Function.
   - `JWT_EXPIRE_MINUTES=60`, `JWT_REFRESH_EXPIRE_DAYS=30`.
   - `TRUST_PROXY_IDENTITY_HEADERS=False` — запрет прямого доверия заголовкам клиента в обход Nginx.
2. **База данных и порядок применения миграций**:
   - Миграция `ProcessingBackend/backend/alembic/versions/021_add_session_active_org_and_user_last_org.py` должна быть применена через `alembic upgrade head` **до** старта обновленной версии бэкенда.
   - Миграция является non-breaking (nullable-колонки `user_sessions.active_org_id` и `users.last_org_id`), безопасна для rolling update.
3. **Nginx конфигурация**:
   - На маршруте `/api/auth/`: отключена проверка JWT (`auth_jwt_enabled off;`), зачищаются заголовки identity.
   - На прикладных маршрутах (`/api/`, `/api/billing/`): проверка JWT по публичному ключу RS256, извлечение токена из `COOKIE=accessToken`, форвардинг заголовков `X-User-Id`, `X-Org-Id`, `X-User-Role`.
   - Перезапуск Nginx требуется только при правках шаблона конфигурации.

---

### 4. Открытые вопросы и архитектурные рекомендации

1. **L4: Поведение сервиса `app1` (IoT Platform REST API) при `org=0`**:
   - При работе суперпользователя на платформенном уровне (`orgId=0`), Nginx форвардит `X-Org-Id: 0` в сервис `app1` (`location ~ ^/api/internal/v1/`).
   - *Рекомендация:* Проверить на стороне сервиса `app1`, что значение `org=0` воспринимается либо как глобальный административный контекст (просмотр устройств всех тенантов), либо возвращает явное требование выбора организации (`403 Forbidden`).
2. **L1: Nonce-store и защита от replay-атак во внешнем JWT Issuer**:
   - Бэкенд генерирует криптографически стойкий `nonce` (UUIDv4) и включает его в подписанную строку HMAC `timestamp.nonce.requestBody`.
   - Для полноценной защиты от replay-атак в распределенной среде внешняя Yandex Cloud Function должна сохранять использованные `nonce` в быстром кэше (например, Redis или YDB) со сроком жизни, равным допустимому окну рассинхронизации `timestamp` (например, 60 секунд).

---

### 5. Итоговый статус задачи

- Все тесты в `MenuBuilder/backend` (201 тест) успешно проходят.
- Статический анализ (`ruff check`, `ruff format`, `pyright`) без единой ошибки и предупреждения.
- Сборка фронтенда `MenuBuilder/frontend` проходит успешно (`npm run build`).
- Деплой на production сервер `176.108.247.249` выполнен и полностью верифицирован smoke-тестами на `dev.leo4.ru`.
