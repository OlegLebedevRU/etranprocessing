...
class IotConsumerCheckpoint(Base):
    """Stores the latest committed consumer checkpoint for event feed streams."""
...
class IotEventInbox(Base):
    """Idempotent technical event inbox deduplicated by event_id."""
    ...
    )  # 'processed', 'duplicate', 'quarantined'


class IotEventQuarantine(Base):
    """Quarantine for contract-defined anomalies (schema mismatch, out-of-order cursor)."""
