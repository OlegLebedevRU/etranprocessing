...
function getTagValue(tags: DeviceTagItem[] | undefined, tagName: string): string | undefined {
  const item = tags?.find((t) => t.tag.toLowerCase() === tagName.toLowerCase());
  return item?.value;
}
...
function renderSystemPlatform(sys?: string) {
  ...
  if (lower.includes("win")) {
    color = "blue";
  } else if (lower.includes("linux") || lower.includes("debian") || lower.includes("ubuntu")) {
    color = "green";
  } else if (lower.includes("esp") || lower.includes("mcu") || lower.includes("stm") || lower.includes("freertos")) {
    color = "orange";
...
export default function DevicePassportTab({
  ...
  const fetchDeviceData = useCallback(async () => {
    ...
    try {
      ...
      const errMsg = err?.message || "Ошибка загрузки данных паспорта устройства";
      setError(errMsg);
  ...
  const isOnline = Boolean(conn?.last_checked_result);
  ...
  // Peer host & port
  const peerAddress = details?.peer_host
    ? `${details.peer_host}${details.peer_port ? `:${details.peer_port}` : ""}`
    : undefined;
      ...
      <Space direction="vertical" size="middle" style={{ width: "100%", marginTop: 8 }}>
        {error && (
          <Alert
            ...
            showIcon
            action={
              <Button size="small" type="primary" danger onClick={fetchDeviceData}>
                Повторить
              </Button>
            }
          />
        ...
        >
          ...
          <Descriptions.Item label="Прикладное ПО (app)">
            {appTag ? <Tag color="blue">{appTag}</Tag> : <Text type="secondary">—</Text>}
          </Descriptions.Item>
        ...
        {/* Блок 2: Сетевое подключение и статус MQTT */}
        <Descriptions
          title="Сетевое подключение и статус MQTT"
          bordered
          size="small"
          column={{ xs: 1, sm: 2, md: 2 }}
        >
          <Descriptions.Item label="Статус соединения">
            <Space wrap size={8}>
              {isOnline ? (
                <Badge status="success" text={<Text strong style={{ color: "#52c41a" }}>Онлайн</Text>} />
              ) : (
                <Badge status="error" text={<Text strong style={{ color: "#ff4d4f" }}>Оффлайн</Text>} />
              )}
          ...
          <Descriptions.Item label="IP-адрес и порт">
            {peerAddress ? <Text code>{peerAddress}</Text> : <Text type="secondary">—</Text>}
          </Descriptions.Item>
        ...
        {/* Блок 3: Данные mTLS сертификата */}
        <Descriptions
          title={
            <Space wrap>
        ...
        {/* Блок 4: Сырой объект JSON (Raw JSON Viewer) */}
        <Card
          size="small"
