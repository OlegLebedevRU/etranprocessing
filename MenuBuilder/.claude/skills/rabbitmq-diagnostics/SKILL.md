...
- **Container**: `rabbitmq` (managed via Docker Compose in `/home/user1/compose.yaml`)
- **Broker Version**: `RabbitMQ 4-management` (Erlang/OTP)
...
ssh -n -o StrictHostKeyChecking=no -i d:\.ssh\id_ed25519 user1@87.242.100.34 "curl -s -u guest:guest http://localhost:15672/rabbitmq/api/overview | jq '{message_stats, object_totals, queue_totals}'"
